...
class Middleware
{
    ...
    {
        $this->queue = [];
        foreach ((array)$middleware as $midd) {
            $this->queue[] = [new $midd, 'handle'];
        }
        return $this;
    }
...
//    public function end()
//    {
//        return $this->carry($this->request);
//    }
//
//    protected function carry($request)
//    {
...
//    {
//        $this->queue[] = [$middleware, 'hanlde'];
//    }
//
//    public function throws(\Closure $throws)
//    {
//        $this->res = $throws;
//    }
    ...
    public function then($after)
    {
        foreach ($this->queue as $middleware) {
            $after = function ($request) use ($after, $middleware) {
                return call_user_func_array($middleware, [$request, $after]);
            };
        }
        $this->request = $after;
        return $this;
    ...
    public function carry($req)
    {
        if ($req instanceof \Closure) {
            return $this->carry($req(app('request')));
        }
        return $req;
    }
